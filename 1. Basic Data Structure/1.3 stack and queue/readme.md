...
* Monotonic Stack 內的元素一定是遞增/遞減排序的
  * 遇到大的直要做處理 -> 用 遞減 的stack
  * 遇到小的直要做處理 -> 用 遞增 的stack
* Monotonic Stack 可以幫助找到 向左/向右 走訪 
  * 第一個 比當前元素 小/大 的元素 (每個元素被pop時的curr會是第一個大/小於自己的數字）
  * 向左向右 比被pop元素 小/大的元素總和 （84. Largest Rectangle in Histogram）
